...
{
    #[Route('/', name: 'index', methods: ['GET'])] // Ceci est une annotation PHP, ici elle permet de définir une route pour le contrôleur et ses paramètres.
    public function index(PersonRepository $pr): Response // Ceci est le contrôleur qui va être appelé lorsque l'utilisateur accède à la page '/'
    {
        ...
        return $this->render('settings/index.html.twig', [ // La méthode render() de la classe AbstractController permet de renvoyer un template avec des données.
    ...
    public function home(string $name = 'Nobody'): Response // Ceci est le contrôleur qui va être appelé lorsque l'utilisateur accède à la page '/'
    {
        return $this->render('home/index.html.twig', [ // La méthode render() de la classe AbstractController permet de renvoyer un template avec des données.
            'name' => $name ?? 'Nobody',
            // Si le paramètre n'est pas renseigné, on lui donne un nom par défaut.
        ]);
    }
}
